...
PlaylistDeprecated::PlaylistDeprecated(QString name, QObject* parent)
...
PlaylistDeprecated::~PlaylistDeprecated()
{
...
void PlaylistDeprecated::setSettings(const PlaylistSettings &settings)
{
    settings_->copyFrom(settings);
    emit changedSettings();
}

const PlaylistSettings& PlaylistDeprecated::getSettings() const
{
    return *settings_;
}

void PlaylistDeprecated::setSoundFileModel(SoundFileTableModel *m)
{
    model_ = m;
...
const SoundFileTableModel *PlaylistDeprecated::getSoundFileModel() const
{
    return model_;
}

bool PlaylistDeprecated::addMedia(const SoundFileRecord &rec)
{
    return addMedia(rec.id);
}

bool PlaylistDeprecated::addMedia(int record_id)
{
    if(model_ == 0)
        return false;
...
const QList<SoundFileRecord *> PlaylistDeprecated::getSoundFileList(bool unique)
{
...
void PlaylistDeprecated::onMediaAboutToBeRemoved(int start, int end)
{
